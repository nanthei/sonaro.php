...
$username_err = $password_err = $confirm_password_err = $email_err = $name_err = $surname_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $username_err = "Vartotojo vardas užimtas.";
                } else{
    ...
    if(empty(trim($_POST["password"]))){
        ...
        $password_err = "Slaptažodis turi turėti mažiausiai vieną didžiąją raidę ir vieną skaičių.";
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        ...
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "Slapažodžiai nesutampa.";
        }
    }
     ...
     if(empty(trim($_POST["surname"]))){
        $surname_err = "Įveskite savo pavardę.";     
    } else{
            ...
            <div class="col-sm-12 col-md-10 col-lg-4" >
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <div class="form-group mb-3 text-center">
                        <input type="submit" class="btn btn-primary m-3" value="Registruoti">
                        <input type="reset" class="btn btn-danger" value="Išvalyti">
                    </div>
                    <p  class="form-group mb-3 text-center">Jau turite vartotojo prisijungimus? <a href="login.php" class="text-danger">Prisijungti</a>.</p>
                </form>
            </div>    
        </div>    
    </div>    
